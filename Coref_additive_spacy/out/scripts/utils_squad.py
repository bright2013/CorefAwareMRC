...
def read_squad_examples(input_file, is_training, version_2_with_negative):
    ...
    for entry in input_data:
        for paragraph in entry["paragraphs"]:
            ...
            #     doc_coref.append(0)
            #     index = paragraph_text[start:].find(token)
            #     tokenindex.append(start + index)
            #     start = start + index + len(token)
            # 标记coref关系
            doc_coref = [0] * len(doc)  # token的coref信息
            # for cluster_index, item in enumerate(clusters):
            #     for index, coref in enumerate(item):
            #         print(cluster_index, coref)
            #         for span_index in range(coref[0], coref[1] + 1):
            #             # print(cluster_index)
...
def convert_examples_to_features(examples, tokenizer, max_seq_length,
    ...
    for (example_index, example) in enumerate(examples):
        ...
        for (doc_span_index, doc_span) in enumerate(doc_spans):
            ...
            print("finish features", unique_id)
            features.append(
                InputFeatures(
                    unique_id=unique_id,
