...
BERT_PRETRAINED_MODEL_ARCHIVE_MAP = {
    ...
    'coref-bert-base': "https://cloud.tsinghua.edu.cn/d/a2eed603edd949b0a663/files/?p=%2FCorefBERT-base%2Fpytorch_model.bin&dl=1",
    'coref-bert-large': "https://cloud.tsinghua.edu.cn/d/a2eed603edd949b0a663/files/?p=%2FCorefBERT-large%2Fpytorch_model.bin&dl=1",
}
...
class BertSelfAttention(nn.Module):
    ...
    def forward(self, hidden_states, attention_mask=None, head_mask=None, encoder_hidden_states=None, encoder_attention_mask=None, coref_matrix=None):
        ...
        if coref_matrix is not None:
            for batch_num in range(attention_scores.shape[0]):
                # attention_scores[batch_num][self.num_attention_heads-1] = coref_matrix[batch_num]  # 把最后一个head与coref_matrix直接替换？
                attention_scores[batch_num] = attention_scores[batch_num] + coref_matrix[batch_num]
                '''maxatt = torch.max(attention_scores[batch_num][self.num_attention_heads - 1])
                attention_scores[batch_num][self.num_attention_heads - 1] = torch.where(coref_matrix[batch_num]>0, maxatt, attention_scores[batch_num][self.num_attention_heads - 1])
                maxatt = torch.max(attention_scores[batch_num][self.num_attention_heads - 1])
                attention_scores[batch_num][self.num_attention_heads - 2] = torch.where(coref_matrix[batch_num]>0, maxatt, attention_scores[batch_num][self.num_attention_heads - 2])'''
        ...
        '''if coref_matrix is not None:
            for batch_num in range(attention_scores.shape[0]):
                attention_scores[batch_num][self.num_attention_heads - 1] = torch.where(coref_matrix[batch_num] > 0, torch.ones(1, dtype=torch.long).to(attention_scores.device), attention_scores[batch_num][self.num_attention_heads - 1].long())
                attention_scores[batch_num][self.num_attention_heads - 2] = torch.where(coref_matrix[batch_num] > 0, torch.ones(1, dtype=torch.long).to(attention_scores.device), attention_scores[batch_num][self.num_attention_heads - 2].long())'''
...
class BertAttention(nn.Module):
    ...
    def forward(self, hidden_states, attention_mask=None, head_mask=None, encoder_hidden_states=None, encoder_attention_mask=None, coref_matrix=None):
        self_outputs = self.self(hidden_states, attention_mask, head_mask, encoder_hidden_states, encoder_attention_mask, coref_matrix=coref_matrix)
...
class BertLayer(nn.Module):
    ...
    def forward(self, hidden_states, attention_mask=None, head_mask=None, encoder_hidden_states=None, encoder_attention_mask=None, coref_matrix=None):
        self_attention_outputs = self.attention(hidden_states, attention_mask, head_mask, coref_matrix=coref_matrix)
...
class BertEncoder(nn.Module):
    ...
    def forward(self, hidden_states, attention_mask=None, head_mask=None, encoder_hidden_states=None, encoder_attention_mask=None, coref_matrix=None):
        ...
        for i, layer_module in enumerate(self.layer):
            ...
            layer_outputs = layer_module(hidden_states, attention_mask, head_mask[i], encoder_hidden_states, encoder_attention_mask, coref_matrix)
...
class BertModel(BertPreTrainedModel):
    ...
    def forward(self, input_ids=None, attention_mask=None, token_type_ids=None, position_ids=None,
                head_mask=None, inputs_embeds=None, encoder_hidden_states=None, encoder_attention_mask=None, coref_matrix=None):
...
#old forward
    ...
    def forward(self, input_ids, attention_mask=None, token_type_ids=None, position_ids=None, head_mask=None,
                start_positions=None, end_positions=None, answer_masks=None, answer_nums=None, doc_coref=None):

        # 生成coref邻接矩阵
        coref_matrix = torch.zeros([doc_coref.shape[0], doc_coref.shape[1], doc_coref.shape[1]])
        coref_matrix = coref_matrix.to(input_ids.device)
        for batch in range(doc_coref.shape[0]):
            clusters = {}
            # 统计clusters
            for i in range(doc_coref.shape[1]):
                cluster_num = doc_coref[batch][i]
                if cluster_num == 0:
                    continue
                if str(cluster_num) not in clusters.keys():
                    clusters[str(cluster_num)] = []
                clusters[str(cluster_num)].append(i)
            for key in clusters.keys():
                cluster = clusters[key]
                for i in range(len(cluster)):
                    for j in range(i, len(cluster)):
                        coref_matrix[batch][cluster[i]][cluster[j]] = 1
        outputs = self.bert(input_ids,
                               attention_mask=attention_mask,
                               token_type_ids=token_type_ids,
                               position_ids=position_ids,
