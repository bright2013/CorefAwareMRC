...
class BertSelfAttention(nn.Module):
    ...
    def forward(self, hidden_states, attention_mask=None, head_mask=None, encoder_hidden_states=None, encoder_attention_mask=None, coref_matrix=None):
        ...
        if encoder_hidden_states is not None:
            ...
            mixed_value_layer = self.value(hidden_states)  # 4*512*1024
        query_layer = self.transpose_for_scores(mixed_query_layer)
        key_layer = self.transpose_for_scores(mixed_key_layer)
        value_layer = self.transpose_for_scores(mixed_value_layer)  # 4*16*512*64
        ...
        if coref_matrix is not None:
            diag_matrix = torch.eye(coref_matrix.shape[-1]).to(coref_matrix.device)
            diag_matrix = diag_matrix.view(1, 1, diag_matrix.shape[0], diag_matrix.shape[1])
            diag_matrix = diag_matrix.repeat(context_layer.shape[0], context_layer.shape[1], 1, 1)
            for batch_num in range(diag_matrix.shape[0]):
                diag_matrix[batch_num][context_layer.shape[1]-1] = diag_matrix[batch_num][context_layer.shape[1]-1] + coref_matrix[batch_num]
            context_layer = torch.matmul(diag_matrix, context_layer)
