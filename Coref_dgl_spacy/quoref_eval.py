...
def evaluate_json(annotations, predicted_answers):
    ...
    for query_id, candidate_answers in annotated_answers.items():
        ...
        if query_id in predicted_answers:
            ...
            logger.warning("Missing prediction for question: {}".format(query_id))
            logger.warning("predicted:{}, gold_answer:{}".format(predicted, gold_answer))
            max_em_score = 0.0
            max_f1_score = 0.0
        if max_f1_score < 0.7 or max_em_score < 0.7:
            logger.warning("Missing prediction for question: {}".format(query_id))
            logger.warning("predicted:{}, gold_answer:{}".format(predicted, gold_answer))
        if max_f1_score > 0.9 or max_em_score > 0.9:
            logger.warning("Correct prediction for question: {}".format(query_id))
            logger.warning("predicted:{}, gold_answer:{}".format(predicted, gold_answer))
